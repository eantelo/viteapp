...
import { cancelInventoryCount, completeInventoryCount, getInventoryCount, reconcileInventoryCount, setInventoryCountLine, type InventoryCountDto, type InventoryCountLineDto, type InventoryCountLineResult } from "@/api/inventoryCountsApi";
...
export function InventoryCountDetailPage() {
  ...
  const [transition, setTransition] = useState<"complete" | "reconcile" | "cancel">();
  ...
  async function performTransition() {
    ...
    try {
      const updated = transition === "complete"
        ? await completeInventoryCount(count.id, count.version)
        : transition === "reconcile"
          ? await reconcileInventoryCount(count.id, count.version)
          : await cancelInventoryCount(count.id, count.version);
      applyCount(updated);
      toast.success(transition === "complete" ? "Toma cerrada; revisa el reporte antes de conciliar" : transition === "reconcile" ? "Inventario conciliado" : "Toma cancelada");
  ...
  return <PageTransition><DashboardLayout breadcrumbs={[{ label: "Panel principal", href: "/dashboard" }, { label: "Tomas de inventario", href: "/inventory-counts" }, { label: count?.countNumber ?? "Detalle" }]} className={PAGE_LAYOUT_CLASS}>
    <div className="w-full max-w-[1320px] space-y-4">
      <header className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-2"><Button variant="ghost" size="icon" onClick={() => navigate("/inventory-counts")}><ArrowLeft /></Button><div><h1 className="text-2xl font-semibold">{count?.countNumber ?? "Toma de inventario"}</h1><p className="text-sm text-muted-foreground">{count ? `${count.warehouseName} · iniciada ${new Date(count.snapshotAt).toLocaleString("es-BO")}` : "Captura física por almacén"}</p></div></div>{count?.status === "Draft" && canManage ? <div className="flex gap-2"><Button variant="outline" onClick={() => setTransition("cancel")}>Cancelar toma</Button><Button onClick={() => setTransition("complete")} disabled={count.countedProducts === 0}><CheckCircle /> Cerrar toma</Button></div> : count?.status === "Completed" && canManage ? <Button onClick={() => setTransition("reconcile")}><CheckCircle /> Conciliar existencias</Button> : null}</header>
      {loading ? <div className="py-16 text-center text-sm text-muted-foreground"><SpinnerGap className="mx-auto mb-2 animate-spin" size={26} />Cargando toma...</div> : error ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-destructive">{error}<Button variant="outline" size="sm" className="ml-3" onClick={() => void load()}>Reintentar</Button></div> : count && <>
        <div className="grid gap-4 md:grid-cols-4"><Card><CardHeader className="pb-2"><CardDescription>Estado</CardDescription><CardTitle className="text-xl">{count.status === "Draft" ? "En curso" : count.status === "Completed" ? "Pendiente de conciliación" : count.status === "Reconciled" ? "Conciliada" : "Cancelada"}</CardTitle></CardHeader></Card><Card><CardHeader className="pb-2"><CardDescription>Productos incluidos</CardDescription><CardTitle className="text-xl">{count.countedProducts}</CardTitle></CardHeader></Card><Card><CardHeader className="pb-2"><CardDescription>Disponibles sin contar</CardDescription><CardTitle className="text-xl">{count.pendingProducts}</CardTitle></CardHeader></Card><Card><CardHeader className="pb-2"><CardDescription>Diferencia neta</CardDescription><CardTitle className="text-xl">{count.differenceUnits == null ? "—" : count.differenceUnits > 0 ? `+${count.differenceUnits}` : count.differenceUnits}</CardTitle></CardHeader></Card></div>
        {count.status === "Draft" && <Card><CardContent className="space-y-2 pt-5"><div className="flex justify-between text-sm"><span>Productos seleccionados para esta toma</span><span className="font-medium">{count.countedProducts} de {count.totalProducts} disponibles</span></div><Progress value={progress} /><p className="flex items-center gap-1 text-sm text-muted-foreground"><Warning />{count.countedProducts === 0 ? "Registra al menos un producto para completar la toma." : `${count.pendingProducts} productos sin registrar quedarán fuera de la conciliación.`}</p></CardContent></Card>}
        {(count.status === "Completed" || count.status === "Reconciled") && <Card><CardHeader><CardTitle>Resumen de conciliación</CardTitle><CardDescription>{count.status === "Reconciled" ? `Los ajustes auditados fueron aplicados${count.reconciledAt ? ` el ${new Date(count.reconciledAt).toLocaleString("es-BO")}` : ""}.` : "El reporte está cerrado; revisa las diferencias y usa Conciliar existencias para aplicar los ajustes auditados."}</CardDescription></CardHeader><CardContent className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4"><Metric label="Sistema al cierre" value={count.systemUnits} /><Metric label="Conteo físico" value={count.physicalCountedUnits} /><Metric label="Faltantes" value={count.missingUnits} /><Metric label="Sobrantes" value={count.surplusUnits} /><Metric label="Coincidencias" value={count.matchingProducts} /><Metric label="Productos faltantes" value={count.shortageProducts} /><Metric label="Productos sobrantes" value={count.surplusProducts} /><Metric label="Con movimientos" value={count.productsWithNetChanges} /></CardContent></Card>}
        <Card><CardHeader className="gap-3 md:flex-row md:items-center md:justify-between"><div><CardTitle>{count.status === "Draft" ? "Captura de productos" : "Detalle del reporte"}</CardTitle><CardDescription>{count.notes || "Sin notas"}</CardDescription></div><div className="relative w-full md:w-80"><MagnifyingGlass className="absolute left-3 top-2.5 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar nombre, SKU o código" /></div></CardHeader><CardContent><div className="overflow-x-auto rounded-lg border"><Table><TableHeader><TableRow><TableHead>Producto</TableHead><TableHead>SKU / código</TableHead>{count.status !== "Draft" && count.status !== "Cancelled" && <><TableHead className="text-right">Sistema</TableHead><TableHead className="text-right">Físico ajustado</TableHead><TableHead className="text-right">Diferencia</TableHead></>}<TableHead className="text-right">{count.status === "Draft" ? "Cantidad física" : "Resultado"}</TableHead></TableRow></TableHeader><TableBody>{filteredLines.length === 0 ? <TableRow><TableCell colSpan={6} className="py-10 text-center text-muted-foreground">No hay productos que coincidan con la búsqueda.</TableCell></TableRow> : filteredLines.map((line) => <TableRow key={line.id}><TableCell><p className="font-medium">{line.productName}</p>{line.countedAt && <p className="text-xs text-muted-foreground">Contado {new Date(line.countedAt).toLocaleString("es-BO")}</p>}</TableCell><TableCell>{line.sku}<p className="text-xs text-muted-foreground">{line.barcode || "Sin código"}</p></TableCell>{count.status !== "Draft" && count.status !== "Cancelled" && <><TableCell className="text-right">{line.systemQuantity}</TableCell><TableCell className="text-right">{line.adjustedCountedQuantity}</TableCell><TableCell className="text-right font-semibold">{line.difference != null && line.difference > 0 ? `+${line.difference}` : line.difference}</TableCell></>}<TableCell className="text-right">{count.status === "Draft" ? <div className="ml-auto flex w-40 gap-2"><Input aria-label={`Cantidad física de ${line.productName}`} type="number" min={0} step={1} value={quantities[line.productId] ?? ""} onChange={(event) => setQuantities((current) => ({ ...current, [line.productId]: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") void saveLine(line); }} disabled={!canManage || Boolean(savingProductId)} /><Button size="icon" aria-label={`Guardar ${line.productName}`} onClick={() => void saveLine(line)} disabled={!canManage || Boolean(savingProductId)}>{savingProductId === line.productId ? <SpinnerGap className="animate-spin" /> : <FloppyDisk />}</Button></div> : <Badge variant={line.result === "Shortage" ? "destructive" : line.result === "Surplus" ? "default" : "secondary"}>{resultLabels[line.result]}</Badge>}</TableCell></TableRow>)}</TableBody></Table></div></CardContent></Card>
      </>}
    </div>
    <AlertDialog open={Boolean(transition)} onOpenChange={(open: boolean) => !open && setTransition(undefined)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{transition === "complete" ? "¿Cerrar la toma?" : transition === "reconcile" ? "¿Conciliar las existencias?" : "¿Cancelar esta toma?"}</AlertDialogTitle><AlertDialogDescription>{transition === "complete" ? `Se incluirán ${count?.countedProducts ?? 0} productos. Los ${count?.pendingProducts ?? 0} productos sin registrar quedarán fuera del reporte y no se modificarán. El cierre todavía no ajusta existencias.` : transition === "reconcile" ? "Se crearán movimientos auditados únicamente para las diferencias de los productos contados. Los productos omitidos no cambiarán. Esta acción no se puede deshacer desde la toma." : "Se descartará el conteo en curso sin modificar las existencias."}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={transitioning}>Volver</AlertDialogCancel><AlertDialogAction disabled={transitioning} onClick={(event: MouseEvent<HTMLButtonElement>) => { event.preventDefault(); void performTransition(); }}>{transitioning && <SpinnerGap className="animate-spin" />}{transition === "complete" ? "Cerrar toma" : transition === "reconcile" ? "Aplicar conciliación" : "Cancelar toma"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
